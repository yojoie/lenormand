...
const MaskedHeading = ({
  ...
  const sync = useCallback(() => {
    ...
    for (let i = 0; i < strokeGlyphRefs.current.length; i += 1) {
      const sg = strokeGlyphRefs.current[i];
      const box = wordRefs.current[i];
      const base = baseRefs.current[i];
      if (!sg || !box || !base) continue;
      sg.setAttribute('x', `${box.offsetLeft}`);
      sg.setAttribute('y', `${base.offsetTop}`);
      sg.style.fontFamily = cs.fontFamily;
      sg.style.fontSize = cs.fontSize;
      sg.style.fontWeight = cs.fontWeight;
      sg.style.fontStyle = cs.fontStyle;
      sg.style.letterSpacing = cs.letterSpacing;
    }
    place();
  }, [place]);
  ...
  useEffect(() => {
    ...
    // Group glyphs by word so clip + stroke for the same word animate together
    const glyphGroups = [];
    for (let i = 0; i < wordRefs.current.length; i++) {
      const group = [glyphRefs.current[i], strokeGlyphRefs.current[i]].filter(Boolean);
      if (group.length) glyphGroups.push(group);
    }
    const glyphs = glyphGroups.flat();
  ...
  return (
    ...
    >
      ...
      {strokeColor && (
        <svg className="masked-heading__stroke" aria-hidden="true" focusable="false">
          {words.map((word, i) => {
            if (strokeWordIndex >= 0 && i !== strokeWordIndex) return null;
            return (
              <text
                key={`stroke-${word}-${i}`}
                ref={el => {
                  strokeGlyphRefs.current[i] = el;
                }}
                fill="none"
                stroke={strokeColor}
