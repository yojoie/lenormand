...
class App {
  ...
  onTouchDown(e) {
    ...
    this.moved = false;
    this.downClientX = this.start;
    this.downClientY = e.touches ? e.touches[0].clientY : e.clientY;
  }
  onTouchMove(e) {
    if (!this.isDown) return;
    const x = e.touches ? e.touches[0].clientX : e.clientX;
    const distance = (this.start - x) * (this.scrollSpeed * 0.025);
    if (Math.abs(x - this.start) > 4) this.moved = true;
  ...
  onTouchUp(e) {
    this.isDown = false;
    this.onCheck();
    if (this.moved || Date.now() - this.downTime > 250 || !this.props.onItemClick) return;
    const clickedIndex = this.getClickedCardIndex(this.downClientX, this.downClientY);
    if (clickedIndex >= 0) {
      this.props.onItemClick(clickedIndex);
    }
  }
  getClickedCardIndex(clientX, clientY) {
    ...
    for (let i = 0; i < this.originalLength; i++) {
      ...
      // 卡片在 3D 空间的 x 坐标
      const planeX = media.x - this.scroll.current - media.extra;
      // 将 3D 坐标映射到屏幕坐标（近似线性的水平映射）
      const scale = rect.width / this.viewport.width;
      const screenX = centerX + planeX * scale;
      const halfW = (media.plane.scale.x * scale) / 2;
      // 卡片高度（考虑 bend 弧度，简化为固定高度判断）
      const halfH = (media.plane.scale.y * scale) / 2;
      // 卡片 y 偏移（bend 导致的下移）
      const H = this.viewport.width / 2;
      const B_abs = Math.abs(this.bend || 0);
      let yOffset = 0;
      if (B_abs > 0) {
        const R = (H * H + B_abs * B_abs) / (2 * B_abs);
        const effectiveX = Math.min(Math.abs(planeX), H);
        const arc = R - Math.sqrt(R * R - effectiveX * effectiveX);
        yOffset = this.bend > 0 ? -arc : arc;
      }
      const screenY = centerY + yOffset * scale - 1.5 * scale; // 相机 y=1.5 的偏移
      if (px >= screenX - halfW && px <= screenX + halfW &&
          py >= screenY - halfH && py <= screenY + halfH) {
        return i;
      }
    }
    return -1;
  ...
  clampScroll() {
    if (!this.medias || !this.medias[0] || this.originalLength > 5) return;
    const width = this.medias[0].width;
    const maxScroll = width * (this.originalLength - 1);
  ...
  onCheck() {
    ...
    let clampedIndex = itemIndex;
    if (this.originalLength <= 5) {
      clampedIndex = Math.max(0, Math.min(this.originalLength - 1, itemIndex));
    }
  ...
  update() {
    ...
    if (this.medias) {
      ...
      // 当卡片数量较少时，隐藏重复的卡片，只显示原始数量的卡片
      if (this.originalLength <= 5) {
        const halfViewport = this.viewport.width / 2;
        const planeHalfW = this.medias[0].plane.scale.x / 2;
        this.medias.forEach((media, index) => {
          const planeX = media.x - this.scroll.current - media.extra;
          // 超出视野左右边界的卡片隐藏
          if (planeX + planeHalfW < -halfViewport || planeX - planeHalfW > halfViewport) {
            media.plane.visible = false;
          } else {
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      app = new App(containerRef.current, {
        ...
        scrollSpeed,
        scrollEase,
        onItemClick: (idx) => onItemClickRef.current && onItemClickRef.current(idx)
      });
    });
