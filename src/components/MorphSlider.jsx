...
class MorphEngine {
  constructor(container, { items, startIndex, reducedMotion, getOptions, onIndexChange, dprCap }) {
    ...
    this.tween = null;
    this.renderer = new Renderer({ alpha: false, antialias: true, dpr: Math.min(window.devicePixelRatio || 1, dprCap) });
  ...
  endDrag() {
    ...
    if (p > 0.4) {
      ...
      this.tween = gsap.to(this.program.uniforms.uProgress, {
        value: 0, duration, ease: 'power2.out', onComplete: () => { this.animating = false; this.tween = null; }
      });
    }
  }
...
export default function MorphSlider({
  ...
  useEffect(() => {
    ...
    const onUp = () => {
      ...
      if (!moved && Date.now() - downTime < 250 && onItemClickRef.current) {
        const current = engineRef.current?.current ?? 0;
        onItemClickRef.current(current);
      }
    };
    ...
    el.addEventListener('pointerleave', onMovePointer);
    return () => {
      ...
      el.removeEventListener('pointerleave', onMovePointer);
    };
  }, []);

  const captions = items.map(it => it.caption).filter(c => c != null);
  const showCaption = showCaptions && captions.length > 0;

  return (
    <div
      className={`morph-slider ${className}`}
      style={{ '--ms-radius': `${radius}px`, '--ms-swap': '0.6s', '--ms-dot': '0.35s' }}
      onMouseEnter={() => setHovering(true)}
      onMouseLeave={() => setHovering(false)}
      {...props}
    >
      <div className="morph-slider-stage" ref={containerRef} tabIndex={0} style={{ borderRadius: radius }} />
      {showCaption && (
        <div className="morph-slider-caption">
          {items.map((it, i) => (
            <span key={i} className={`morph-slider-caption-text ${i === index ? 'is-active' : ''}`}>
              {it.caption}
            </span>
          ))}
        </div>
      ...
      {showControls && (
        <div className="morph-slider-controls">
          <button className="morph-slider-btn" onClick={handlePrev} aria-label="Previous">&lsaquo;</button>
          <button className="morph-slider-btn" onClick={handleNext} aria-label="Next">&rsaquo;</button>
        </div>
      )}
      {showIndicators && (
